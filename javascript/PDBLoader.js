...
THREE.PDBLoader.prototype = {
		...
		for( var i = 0, il = lines.length; i < il; ++ i ) {
			...
			if( lines[i].substr(0,4)=="ATOM" || lines[i].substr(0,6)=="HETATM" ) {
				...
				if( bonds[i-len] ) {
					...
					bonds[i-len].count = parseInt( lines[i].substr(35,1) );
					bonds[i-len].reg = parseInt( lines[i].substr(37,5) );
				}
				else {
					bonds[i-len].count = 0;
				}
	...
	createModel: function ( json, callback ) {
		...
		for ( var i = 0; i < bonds.length; i ++ ) {
			...
			geometryBonds.vertices.push( bond.count );
			geometryBonds.vertices.push( bond.reg );
		}
